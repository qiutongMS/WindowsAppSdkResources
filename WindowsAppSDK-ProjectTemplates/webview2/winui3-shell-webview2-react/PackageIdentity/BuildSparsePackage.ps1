...
$script:Config = @{
    IdentityName   = "Microsoft.WinShell.SparseApp"
    SparseMsixName = "WinShellSparse.msix"
    CertPrefix     = "WinShellSparse"
    CertSubject    = 'CN=WinShell Dev, O=WinShell, L=Redmond, S=Washington, C=US'
    CertValidMonths = 12
}
...
if ($needNewCert) {
    ...
    $friendlyName = "WinShell Dev Sparse Cert Create=$now"
    $keyFriendly = "WinShell Dev Sparse Key Create=$now"
...
# Determine output directory - place outputs one level up from the PackageIdentity folder
# This yields: WindowsAppSDK-ProjectTemplates\x64\Release (parent of this PackageIdentity folder)
...
try {
    ...
    # Pipeline/CI paths are not used for this template; always use local dev publisher
    $shouldUseDevPublisher = $true
