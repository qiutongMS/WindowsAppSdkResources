# WinShell sparse package identity

This document describes how to build, sign, register, and consume the shared sparse MSIX package that grants package identity to select Win32 components of WinShell.

## Package overview

The sparse package lives under `src/PackageIdentity`. It produces a payload-free MSIX whose `Identity` matches `Microsoft.WinShell.SparseApp`. The manifest contains one entry per Win32 surface that should run with identity.
...
The script determines the proper `makeappx.exe` for the host build machine and creates `WinShellSparse.msix` in `{repo}\<Platform>\<Configuration>`.


> After packaging finishes, the helper also emits `src/PackageIdentity/.user/WinShellSparse.publisher.txt`. This file mirrors the publisher string Windows will see once the sparse package is registered, which downstream projects can read to stay in sync when generating their own manifests.
...
1. Artifacts are stored in `src/PackageIdentity/.user/WinShellSparse.certificate.sample.*` (`.cer` and `.thumbprint`).
2. Install the `.cer` into `CurrentUser` → `TrustedPeople` (and `TrustedRoot`, if necessary) so Windows trusts the signature:

   ```powershell
   $repoRoot = "C:/git/WinShell"
   Import-Certificate -FilePath "$repoRoot/src/PackageIdentity/.user/WinShellSparse.certificate.sample.cer" -CertStoreLocation Cert:\CurrentUser\TrustedPeople
   ```
...
After `WinShellSparse.msix` is generated:

```powershell
# First time registration
$repoRoot = "C:/git/WinShell"
$outputRoot = Join-Path $repoRoot "x64/Release"
Add-AppxPackage -Path (Join-Path $outputRoot "WinShellSparse.msix") -ExternalLocation $outputRoot
...
Get-AppxPackage -Name Microsoft.WinShell.SparseApp | Remove-AppxPackage
```
...
1. Add a new `<Application>` entry inside `src/PackageIdentity/AppxManifest.xml`. Use a unique `Id` (for example `WinShell.MyModuleUI`) and set `Executable` to the Win32 binary relative to the `-ExternalLocation` root.
...
3. Embed a sparse identity manifest in the Win32 binary so it binds to the MSIX identity at runtime. The manifest must declare an `<msix>` element with `packageName="Microsoft.WinShell.SparseApp"`, `applicationId` matching the `<Application Id>`, and a `publisher` that matches the sparse package. Keep the manifest’s publisher in sync with `src/PackageIdentity/.user/WinShellSparse.publisher.txt` (emitted by `BuildSparsePackage.ps1`). See `src/modules/imageresizer/ui/ImageResizerUI.csproj` for an example that points `ApplicationManifest` to `ImageResizerUI.dev.manifest` for local builds and switches to `ImageResizerUI.prod.manifest` when `$(CIBuild)` is `true`.
4. Register or re-register the sparse package so Windows learns about the new application Id.
5. To launch the Win32 surface with identity, use the `shell:AppsFolder` activation form (for example: `shell:AppsFolder\Microsoft.WinShell.SparseApp_<PackageFamilyName>!WinShell.MyModuleUI`) or activate it via `IApplicationActivationManager::ActivateApplication` using the same AppUserModelID.

   - For locally built packages, resolve the `<PackageFamilyName>` with `Get-AppxPackage -Name Microsoft.WinShell.SparseApp | Select-Object -ExpandProperty PackageFamilyName`.
   - Store-distributed builds use `Microsoft.WinShell.SparseApp_8wekyb3d8bbwe`. Local developer builds created by this script typically use a different family name derived from the dev certificate.
